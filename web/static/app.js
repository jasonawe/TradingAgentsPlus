(() => {
  ...
  const setupView = $("setup-view"), analysisView = $("analysis-view"), activeView = $("active-view"), reportView = $("report-view"), libraryView = $("library-view"), settingsView = $("settings-view"), assetView = $("asset-detail-view"), form = $("analysis-form");
  ...
  function applyTranslations() { document.documentElement.lang = i18n.locale; document.title = t("app.title"); document.querySelectorAll("[data-i18n]").forEach((node) => { node.textContent = t(node.dataset.i18n); }); document.querySelectorAll("[data-i18n-placeholder]").forEach((node) => { node.placeholder = t(node.dataset.i18nPlaceholder); }); if (state.config) { renderFormOptions(state.config); toggleCryptoAnalyst(); } renderPhases(); renderHistory(state.history); if (!libraryView.hidden) renderLibrary(); if (state.runRecord) renderRunHeader(state.runRecord); }
  ...
  const ROUTES = { setup: "/", analysis: "/analysis", active: "/active", library: "/reports", settings: "/settings" };
  ...
  function renderRoute(route) { if (route.view === "report") { prepareReport(route.reportId); loadReport(route.reportId, { route: false }); return; } if (route.view === "asset" && route.symbol) { showAssetDetail(route.symbol); return; } if (route.view === "setup") showSetup(); else if (route.view === "analysis") showAnalysis(); else if (route.view === "active") showActive(); else if (route.view === "library") showLibrary(); else if (route.view === "settings") showSettings(); }
  ...
  function renderReport(report) { switchView("report"); $("report-panel").hidden = false; $("report-title").textContent = t("report.title", { ticker: report.ticker || t("report.decisionReport") }); const request = state.runRecord?.request || {}; const metadata = [["report.signal", formatRating(report.rating || report.signal)], ["report.date", report.analysis_date || request.analysis_date], ["report.asset", report.asset_type === "crypto" ? t("assets.crypto") : report.asset_type === "stock" ? t("assets.stock") : report.asset_type], ["report.depth", report.research_depth || request.research_depth], ["report.provider", report.provider || request.provider], ["report.quickModel", report.quick_model || request.quick_model], ["report.deepModel", report.deep_model || request.deep_model], ["report.language", languageLabel(report.output_language || request.output_language)], ["report.source", report.source === "legacy" ? t("library.legacy") : report.source], ["report.dataStatus", report.data_status || t("common.unknown")], ["report.snapshot", report.data_snapshot_id || t("common.notGenerated")]].filter(([, value]) => value !== null && value !== undefined && value !== ""); $("report-meta").innerHTML = metadata.map(([label, value]) => `<div><dt>${escapeHtml(t(label))}</dt><dd>${escapeHtml(value)}</dd></div>`).join(""); $("download-report").onclick = () => { window.location.href = `/api/history/${encodeURIComponent(report.report_id)}/download`; }; const summary = report.executive_summary || ""; $("executive-summary").hidden = !summary; const summaryContent = $("executive-summary-content"); summaryContent.innerHTML = summary ? renderReportMarkdown(report.executive_summary_html, summary) : ""; wrapReportTables(summaryContent); $("detail-report").open = !summary; const sections = REPORT_GROUPS.map(([title, group, sectionList]) => { const blocks = sectionList.map(([label, key]) => { const markdown = report[group]?.[key] || ""; const html = report[`${group}_html`]?.[key]; return markdown ? `<article class="report-section"><h3>${escapeHtml(t(label))}</h3>${renderReportMarkdown(html, markdown)}</article>` : ""; }).join(""); return blocks ? `<section><h2>${escapeHtml(t(title))}</h2>${blocks}</section>` : ""; }).join(""); const reportContent = $("report-content"); reportContent.innerHTML = sections || renderReportMarkdown(report.complete_report_html, report.complete_report || t("library.metadataUnavailable")); wrapReportTables(reportContent); }
  function historyItem(record, library) { const status = formatRating(record.rating || record.signal) || (record.status ? t(`status.${record.status}`) : t("status.report")); const freshness = record.data_status ? t("history.data", { value: record.data_status }) : ""; const metadata = [record.analysis_date, record.asset_type ? (record.asset_type === "crypto" ? t("assets.crypto") : t("assets.stock")) : "", record.provider || "", freshness].filter(Boolean).join(" · "); return `<article class="${library ? "library-item" : "history-item"}"><button type="button" data-report-id="${escapeHtml(record.report_id)}"><span class="history-ticker">${escapeHtml(record.ticker || t("history.unknown"))}</span><span class="history-meta">${escapeHtml(metadata)}</span></button><span class="history-signal">${escapeHtml(status)}</span><span class="history-preview">${escapeHtml(record.decision_preview || "")}</span><button class="text-button" type="button" data-report-id="${escapeHtml(record.report_id)}">${escapeHtml(t("actions.open"))}</button></article>`; }
  ...
  function renderWatchlist(items, quotes = {}) { state.watchlist.items = items || []; state.watchlist.quotes = quotes || {}; const list = $("watchlist-list"); if (!list) return; if (!state.watchlist.items.length) { list.innerHTML = `<p class="muted watchlist-empty">${escapeHtml(t("watchlist.empty"))}</p>`; return; } const rows = state.watchlist.items.map((item) => { const quote = quotes[item.symbol] || {}; const analysis = latestAnalysisFor(item.symbol); const value = quote.price == null ? t("watchlist.noQuote") : `${escapeHtml(Number(quote.price).toLocaleString(undefined, { maximumFractionDigits: 6 }))} ${escapeHtml(quote.currency || "")}`; const change = quote.change_percent == null ? "" : `<span class="${Number(quote.change_percent) >= 0 ? "quote-up" : "quote-down"}">${Number(quote.change_percent) >= 0 ? "+" : ""}${escapeHtml(Number(quote.change_percent).toFixed(2))}%</span>`; const source = quote.source ? `${escapeHtml(t("watchlist.source", { value: quote.source }))} · ${quoteFreshness(quote)}` : escapeHtml(quote.error?.message || t("watchlist.dataSourceUnavailable")); const quoteTime = quote.quote_time || quote.as_of || quote.fetched_at; const quoteMeta = quoteTime ? `${source} · ${escapeHtml(new Date(quoteTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }))}` : source; const identity = i18n.assetIdentity(quote); const analysisMeta = analysis ? `${escapeHtml(analysis.analysis_date || analysis.generated_at || "")} · ${escapeHtml(formatRating(analysis.rating || analysis.signal) || t("status.completed"))}` : t("watchlist.noAnalysis"); const preview = analysis?.decision_preview ? cleanSummary(analysis.decision_preview) : t("watchlist.analysisHint"); const reportAction = analysis ? `<button type="button" class="text-button" data-report-id="${escapeHtml(analysis.report_id)}">${escapeHtml(t("actions.viewReport"))}</button>` : ""; const html = `<article class="watchlist-row"><div class="watchlist-asset"><span class="symbol">${escapeHtml(item.symbol)}</span><span class="asset-type">${escapeHtml(t(item.asset_type === "crypto" ? "assets.crypto" : "assets.stock"))}</span><span class="asset-identity"><span>${escapeHtml(t("watchlist.assetName", { value: identity.name }))}</span><span>${escapeHtml(t("watchlist.exchange", { value: identity.exchange }))}</span></span></div><div class="watchlist-quote"><div class="quote-value">${value} ${change}</div><span class="quote-meta">${quoteMeta}</span></div><div class="watchlist-analysis"><span class="analysis-title">${escapeHtml(t("watchlist.latestAnalysis", { value: analysisMeta }))}</span><span class="analysis-preview">${escapeHtml(preview)}</span></div><div class="watchlist-actions"><button type="button" class="text-button watchlist-analyze" data-analyze-symbol="${escapeHtml(item.symbol)}" data-analyze-asset="${escapeHtml(item.asset_type)}">${escapeHtml(t("actions.startAnalysis"))}</button>${reportAction}<button type="button" class="text-button watchlist-toggle-chart" data-toggle-chart="${escapeHtml(item.symbol)}" data-toggle-chart-asset="${escapeHtml(item.asset_type)}" aria-expanded="false">${escapeHtml(t("watchlist.toggleChart"))}</button><button type="button" class="icon-button" data-remove-watchlist="${escapeHtml(item.id)}" data-version="${escapeHtml(state.watchlist.version)}" aria-label="${escapeHtml(`${t("actions.remove")} ${item.symbol}`)}" title="${escapeHtml(t("actions.remove"))}">×</button></div></article>`; return { symbol: item.symbol, html }; }); const expandedSymbols = new Set(); const oldRows = list.querySelectorAll(".watchlist-row[data-expanded='true']"); oldRows.forEach(row => { const btn = row.querySelector("[data-toggle-chart]"); if (btn) expandedSymbols.add(btn.dataset.toggleChart); }); const oldPanels = Array.from(list.querySelectorAll(".watchlist-chart")); oldPanels.forEach(p => p.remove()); list.innerHTML = rows.map(r => r.html).join(""); if (window.WatchlistChart && expandedSymbols.size) { expandedSymbols.forEach(symbol => { const btn = list.querySelector(`[data-toggle-chart="${symbol}"]`); if (btn) window.WatchlistChart.expand(btn); }); }  bindReportLinks(list); list.querySelectorAll("[data-analyze-symbol]").forEach((button) => button.addEventListener("click", () => { $("ticker").value = button.dataset.analyzeSymbol; $("asset-type").value = button.dataset.analyzeAsset; toggleCryptoAnalyst(); navigate("analysis"); $("analysis-form").scrollIntoView({ behavior: "smooth", block: "start" }); })); list.querySelectorAll("[data-remove-watchlist]").forEach((button) => button.addEventListener("click", async (event) => { event.preventDefault(); event.stopPropagation(); const row = button.closest(".watchlist-row"); if (!row) return; const symbol = row.querySelector(".symbol")?.textContent || ""; const confirmed = await openConfirmModal({ title: t("modal.confirmRemoveTitle"), message: t("watchlist.confirmRemove", { value: symbol }), confirmText: t("actions.confirmRemove"), cancelText: t("actions.cancel"), danger: true }); if (!confirmed) return; button.disabled = true; try { await api(`/api/watchlist/items/${encodeURIComponent(button.dataset.removeWatchlist)}?version=${encodeURIComponent(button.dataset.version)}`, { method: "DELETE" }); await loadWatchlist(); } catch (error) { $("watchlist-error").textContent = localizeError(error.message); button.disabled = false; } })); list.querySelectorAll(".watchlist-row").forEach((row) => row.addEventListener("click", (event) => { if (event.target.closest(".watchlist-actions, .watchlist-chart")) return; const symbol = row.querySelector(".symbol")?.textContent || ""; if (symbol) navigate("asset", { symbol }); })); }
  ...
  $("refresh-quotes").addEventListener("click", () => quoteRefreshController?.refresh()); if (window.WatchlistChart) window.WatchlistChart.attach($("watchlist-list")); 
