(() => {
  ...
  const ROUTES = { setup: "/", analysis: "/analysis", active: "/active", scheduled: "/scheduled", "scheduled-history": "/scheduled/history", library: "/reports", settings: "/settings", alerts: "/alerts", notes: "/notes", "agent-audit": "/agent-audit", harness: "/harness" };
...
document.addEventListener("click", (event) => { const retryBtn = event.target.closest("[data-retry-run]"); if (retryBtn) { event.preventDefault(); retryRun(retryBtn.dataset.retryRun); } });
  ...
  document.querySelectorAll(".nav-primary a").forEach((link) => link.addEventListener("click", (event) => {
    ...
    if (!view) return;
    event.preventDefault();
    if (["setup", "analysis", "active", "scheduled", "library", "settings", "alerts", "notes", "agent-audit", "harness"].includes(view)) navigate(view);
  }));
