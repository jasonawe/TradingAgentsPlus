(function (root) {
  ...
  async function saveForm(event) { event.preventDefault(); const symbol = $("scheduled-symbol")?.value; const cron = $("scheduled-cron")?.value.trim(); const note = $("scheduled-note")?.value.trim() || null; const assetType = $("scheduled-asset-type")?.value || "stock"; if (!symbol || !cron || !state.formCronValid) { setError(!state.formCronValid ? t("scheduler.invalidCron") : t("scheduler.formError"), "scheduled-form-error"); return; } const submit = event.submitter; if (submit) submit.disabled = true; const body = { symbol, asset_type: assetType, cron_expression: cron, note }; const path = state.formJob ? `/api/scheduled/jobs/${encodeURIComponent(state.formJob.id)}` : "/api/scheduled/jobs"; state.busy.add("form"); try { await request(path, { method: state.formJob ? "PATCH" : "POST", body: JSON.stringify(body) }); closeForm(); await load(); } catch (error) { setError(error.message || t("scheduler.formError"), "scheduled-form-error"); } finally { state.busy.delete("form"); if (submit?.isConnected) submit.disabled = false; } }
