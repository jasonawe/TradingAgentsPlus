...
async function load({ silent = false } = {}) {
  ...
  function formMarkup(job = null) {
    ...
    const titleText = esc(job ? t("scheduler.edit") : t("scheduler.add"));
    const icon = job ? "&#9998;" : "&#9201;";
    return `<div class="modal-overlay" data-scheduled-close-form></div><div class="modal-dialog scheduled-form-dialog" role="dialog" aria-modal="true" aria-labelledby="scheduled-form-title"><div class="modal-header"><h3 class="modal-title"><span class="modal-title-icon" aria-hidden="true">${icon}</span>${titleText}</h3><button type="button" class="modal-close" data-scheduled-close-form aria-label="关闭"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div><div class="modal-body"><form id="scheduled-form" novalidate><div class="form-field"><label for="scheduled-symbol"><span>${esc(t("scheduler.asset"))}</span></label><select id="scheduled-symbol" required>${options || `<option value="">${esc(t("scheduler.watchlistEmpty"))}</option>`}</select></div><div class="form-field"><label for="scheduled-cron"><span>${esc(t("scheduler.cron"))}</span></label><input id="scheduled-cron" required autocomplete="off" value="${esc(job?.cron_expression || "0 9 * * 1-5")}" aria-describedby="scheduled-cron-hint scheduled-cron-error" /><small id="scheduled-cron-hint" class="form-hint">${esc(t("scheduler.cronHint"))}</small><p id="scheduled-cron-error" class="form-error" role="alert" aria-live="polite"></p><ul id="scheduled-cron-preview" class="scheduled-preview" aria-live="polite"></ul></div><div class="form-field"><label for="scheduled-note"><span>${esc(t("scheduler.note"))}</span></label><textarea id="scheduled-note" rows="3" maxlength="500" placeholder="&#65279;">${esc(job?.note || "")}</textarea></div><p id="scheduled-form-error" class="form-error" role="alert" aria-live="polite"></p></form></div><div class="modal-actions"><button type="button" class="button button-secondary" data-scheduled-close-form>${esc(t("actions.cancel"))}</button><button type="submit" class="button button-primary" form="scheduled-form">${esc(t("scheduler.save"))}</button></div></div>`;
  }
