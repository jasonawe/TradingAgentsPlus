...
def test_sse_emits_envelopes_and_last_event_id_wins(harness):
    ...
    with TestClient(app) as client:
        ...
        assert all("run_id" in data and isinstance(data["seq"], int) and "timestamp" in data for data in _sse_data(stream.text))
        runner.release.set()
    manager.shutdown()
...
def test_history_detail_and_download_are_allowlisted(harness):
    ...
    with TestClient(app) as client:
        ...
        assert detail.json()["complete_report"] == "# report\n"
