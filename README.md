...
| 定时任务 | `/scheduled` | 为关注资产配置 Cron、启停任务、立即运行，顶部按钮可跳到独立分页的触发历史 |
| 触发历史 | `/scheduled/history` | 独立分页页：按任务 / 状态筛选全部触发，附带参数来源（公共默认 / 上次成功 / 全局默认）、耗时和错误复制 |
...
`/scheduled` 页面在初始化时使用浏览器侧共享行情缓存（`TradingAgentsQuotes.fetch`，30 秒内存 TTL + 进行中请求合并）合并并发请求，并把首屏渲染与行情加载解耦：表格、按钮、状态先渲染，资产名 / 交易所行情到达后自动二次渲染，因此首屏不再被 `yfinance` 行情阻塞。`/scheduled/history`、`/reports` 等列表也走同一套共享缓存，避免对相同标的重复请求。
