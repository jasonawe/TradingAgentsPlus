...
TradingAgents also includes a local browser console for starting one analysis at a time, watching agent progress, and reopening completed reports. Install the package from the repository, then launch the loopback-only server:
...
The console supports English and Simplified Chinese. It follows the browser locale on first load, and the language button in the header switches the entire interface. The selection is saved in the browser and is sent with each new run so the generated report uses the same language (`English` or `Chinese`). Existing report text is displayed as generated and is not machine-translated when switching the interface.
...
Active run state and live event history are held in memory. Completed reports are persisted on disk under the configured results directory (`TRADINGAGENTS_RESULTS_DIR`, or the default `~/.tradingagents/logs/web_reports`) and can be reopened from the console's recent-runs history after the server restarts. The console serves reports generated by the web UI and compatible legacy report directories; it does not expose arbitrary filesystem paths.
